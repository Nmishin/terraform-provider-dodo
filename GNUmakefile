...
install: vet fmtcheck
	go install -ldflags="-X github.com/Nmishin/terraform-provider-dodo/main.version=$(VERSION)"

build: vet fmtcheck
	@if $(INCLUDE_VERSION_IN_FILENAME); then \
	    go build -ldflags="-X github.com/Nmishin/terraform-provider-dodo/main.version=$(VERSION)" -o terraform-provider-dodo_$(VERSION); \
		echo "==> Successfully built terraform-provider-dodo_$(VERSION)"; \
	else \
		go build -ldflags="-X github.com/Nmishin/terraform-provider-dodo/main.version=$(VERSION)" -o terraform-provider-dodo; \
		echo "==> Successfully built terraform-provider-dodo"; \
	fi
...
clean-dev:
	...
	@rm -f terraform-provider-dodo_$(DEV_VERSION)

build-dev: clean-dev
	@echo "==> Building development version ($(DEV_VERSION))"
	go build -gcflags="all=-N -l" -o terraform-provider-dodo_$(DEV_VERSION)
...
update-go-client:
	@echo "==> Updating the dodo-go client to $(DODO_GO_VERSION)"
	go get github.com/Nmishin/dodo-go@$(CLOUDFLARE_GO_VERSION)
	go mod tidy
